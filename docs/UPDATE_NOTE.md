...
## 版本 2.3.0（当前版本）- 2026-06-17

- **飞书通知**：新增飞书机器人通知通道，支持 Webhook URL 与签名密钥配置、测试发送、清空配置；监控项可单独启用飞书通知
- **通知逻辑统一**：邮件、Webhook、飞书共用统一的监控通知事件模型，支持状态变化、重试后恢复、监控创建、监控删除等通知场景
- **监控创建/删除通知**：新增监控创建与删除事件通知；只有对应通道开关开启的监控会发送通知，避免各通道重复写判断逻辑
- **数据库与前端配置**：新增 `monitors.feishu_notification` 字段及兼容迁移；添加监控弹窗支持飞书通知开关；设置页新增飞书配置入口
- **后端结构优化**：拆分 `server.ts` 中的检测、通知、模板、状态条、分组路由、公开 API、设置 API 等逻辑；`server/` 按 `core/`、`features/`、`routes/` 进行粗粒度归类，入口文件更聚焦应用装配
- **部署文档修复**：修正 Docker Compose 示例，改为挂载 `./data:/app/data` 并使用 `/app/data/config.json`，避免宿主机 `config.yml` 不存在时被 Docker 创建成目录
- **配置与体验**：监控检测间隔上限调整为 7200 秒；README 与项目规则文档同步更新飞书通知和后端目录规范

## 版本 2.1.0 - 2026-02-09

- **HTTPS/SSL 证书剩余天数**：HTTP 类型监控在检测 HTTPS 目标时自动读取对端证书有效期，计算并存储剩余天数（`monitors.ssl_days_remaining`）；管理页监控卡片与详情弹窗中展示「剩余 N 天」或「已过期」，非 HTTPS 不显示
- **检测与接口**：HTTP 检测的响应时间改为按单次请求计时，避免 HEAD 回退 GET 时两次往返时间累加导致延迟虚高；API 返回监控列表与详情时包含 `ssl_days_remaining`
- **分组状态计算**：管理页与公开状态页中，分组健康/降级/故障状态只统计未暂停的监控，已暂停项不参与计算
- **UI 与样式**：全部检测/分组/设置/添加监控/退出等按钮增加 `title` 或图标；超小屏仅保留图标；退出按钮改为带图标；统计卡片文案「24小时平均可用率」简化为「24小时平均」；详情弹窗与监控卡片 SSL 证书展示样式与占位布局
- **CI**：GitLab CI 构建阶段在推送镜像后增加清理步骤，删除本地已推送的带版本号镜像以释放空间
